...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .photo-preview { margin-top: 15px; max-width: 200px;  border: 1px solid #000; }
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    @if($user->photo)
                        <div style="margin-bottom: 10px;">
                            <img src="{{ asset($user->photo) }}" alt="Current photo" class="photo-preview" style="display: block;">
                            <small style="color: #000;">Current photo</small>
                        </div>
                    @endif
                    <label for="photo" class="photo-upload-label">Change Photo</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="username" class="form-control @error('username') error @enderror" value="{{ old('username', $user->username) }}" required>
                    @error('username')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="password" id="password" name="password" class="form-control @error('password') error @enderror" placeholder="Enter new password or leave blank">
                    @error('password')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="age" name="age" class="form-control @error('age') error @enderror" value="{{ old('age', $user->age) }}">
                    @error('age')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="divisi" name="divisi" class="form-control @error('divisi') error @enderror" value="{{ old('divisi', $user->divisi) }}" required>
                    @error('divisi')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="badge" name="badge" class="form-control @error('badge') error @enderror" value="{{ old('badge', $user->badge) }}">
                    @error('badge')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-control @error('description') error @enderror">{{ old('description', $user->description) }}</textarea>
                    @error('description')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="level" name="level" class="form-control @error('level') error @enderror" required>
                        <option value="normal_user" {{ old('level', $user->level) == 'normal_user' ? 'selected' : '' }}>Normal User</option>
