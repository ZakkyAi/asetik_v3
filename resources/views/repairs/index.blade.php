    ...
    <nav class="navbar">
        <div class="navbar-content">
            ...
            <div class="navbar-menu">
                ...
                <a href="{{ route('repairs.index') }}" class="active">Repairs</a>
            </div>
            <div class="user-info">
    ...
    <div class="container">
        <div class="page-header">
            ...
            @if(auth()->user()->level === 'admin')
                <a href="{{ route('repairs.create') }}" class="btn">➕ Add New Repair</a>
            @endif
        </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $repair->id_record }}</td>
                            <td>{{ Str::limit($repair->note, 50) }}</td>
                            <td>{{ $repair->created_at ? \Carbon\Carbon::parse($repair->created_at)->format('Y-m-d H:i') : 'N/A' }}</td>
                            <td>
                                <div class="actions">
                                    <a href="{{ route('repairs.show', $repair->id_repair) }}" class="btn btn-sm btn-view">View</a>
                                    @if(auth()->user()->level === 'admin')
                                        <a href="{{ route('repairs.edit', $repair->id_repair) }}" class="btn btn-sm btn-edit">Edit</a>
                                        <form method="POST" action="{{ route('repairs.destroy', $repair->id_repair) }}" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
